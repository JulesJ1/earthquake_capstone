...
def filter_magnitude():
    ...
    st.session_state['data'] = st.session_state['data'][st.session_state['data']['magnitude'] > magnitude]
...
def filter_date():
    ...
    with data_filter:
        ...
        with start:
            start_date = st.date_input('start date',value=datetime.now() - timedelta(hours=6))

            start_time = st.time_input('start time',value=datetime.now() - timedelta(hours=6))
        with end:
            end_date = st.date_input('end date')
            end_time = st.time_input('end time')
        #st.button(label='filter date',
        #          on_click=load_data,
        #          args=[f'{start_date} {start_time}',f'{end_date} {end_time}']
        #)
        if st.button(label='filter date'):
            start_datetime = f'{start_date} {start_time}'
            end_datetime = f'{end_date} {end_time}'
            st.session_state['data'] = load_data(f'{start_date} {start_time}',f'{end_date} {end_time}')
