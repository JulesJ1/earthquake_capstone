...
def additional_transformations():
    st.session_state['data']['normalised_mag'] = (
            (st.session_state['data']['magnitude'] -
             st.session_state['data']['magnitude'].min()) /
            (st.session_state['data']['magnitude'].max() -
             st.session_state['data']['magnitude'].min())
        )
    st.session_state['data']['colour'] = st.session_state['data'][
...
def second_tab():
    ...
    with col1:
        series = st.session_state['data']['magnitude'].groupby(
            st.session_state['data']['time'].dt.hour
            ).count()
        df2 = pd.DataFrame({
